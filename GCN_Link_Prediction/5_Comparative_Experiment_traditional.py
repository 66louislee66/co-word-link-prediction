# Compare and contrast traditional machine learning algorithms
# * import module
import pandas as pd
import numpy as np
import torch
...
for data_type in dataset:
    df_LDA = pd.read_csv(r'./data/4_webofsci_{}_tf-idf_features.csv'.format(data_type), index_col = 0)
...
balanced_data.to_csv(r'./test/random_forest')
...
# * Define model examples
nb_model = GaussianNB()
lr_model = linear_model.LogisticRegression(solver='lbfgs',max_iter=1000)
rf_model = ensemble.RandomForestClassifier(n_estimators=10, n_jobs=-1)
...
# * Master functions for training and testing models
def main(clfs):
    # Training dataset
    ...
    # Initialise the training set x, y matrix and construct
